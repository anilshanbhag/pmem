...
#define BASE_DIR "/mnt/pmem13/dbops/test/ssb/data/"

#if SF == 1
#define DATA_DIR BASE_DIR "s1_columnar/"
...
#define D_LEN 2556
#elif SF == 10
#define DATA_DIR BASE_DIR "s10_columnar/"
...
#define D_LEN 2556
#elif SF == 20
#define DATA_DIR BASE_DIR "s20_columnar/"
...
#define D_LEN 2556
#elif SF == 100
#define DATA_DIR BASE_DIR "s100_columnar/"
...
#define D_LEN 2556
#elif SF == 500
#define DATA_DIR BASE_DIR "s500_columnar/"
#define LO_LEN 3000028334
#define P_LEN 1800000
#define S_LEN 1000000
#define C_LEN 15000000
#define D_LEN 2556
#elif SF == 1000
#define DATA_DIR BASE_DIR "s1000_columnar/"
#define LO_LEN 5999989813
...
template<typename T>
T* loadColumn(string col_name, size_t num_entries) {
...
template<typename T>
T* loadColumn(string col_name, size_t num_entries, string type) {
  size_t buffer_size = sizeof(T) * num_entries;
  string path = DATA_DIR + lookup(col_name);
  ...
  cout << path << endl;
  if (file_exists(path)) {
    pmemaddr = (char*)pmem_map_file(path.c_str(), 0,
        0, 0666, &mapped_len, &is_pmem);
  } else {
    perror("pmem_map_file does not exit");
    exit(1);
  }
  ...
  if (type == "pmem") {
    ...
    T* col = new T[num_entries];
    memcpy(col, buf, num_entries * sizeof(T));
/*    string filename = DATA_DIR + lookup(col_name);*/
    //ifstream colData (filename.c_str(), ios::in | ios::binary);
    //if (!colData) {
      //perror("FileError");
