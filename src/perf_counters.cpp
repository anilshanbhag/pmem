...
#include "pcm/cpucounters.h" /* intel perf counters monitor */
...
#if PER_CORE==0
        (*out) << "BytesFromMC "
               << getBytesReadFromMC(before_state, after_state) << endl;
        (*out) << "BytesWrittenToMC "
               << getBytesWrittenToMC(before_state, after_state) << endl;
        (*out) << "BytesReadFromPMM "
               << getBytesReadFromPMM(before_state, after_state) << endl;
        (*out) << "getBytesWrittenToPMM "
               << getBytesWrittenToPMM(before_state, after_state) << endl;
#endif
