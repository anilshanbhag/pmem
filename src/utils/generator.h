 ...
 */
void
knuth_shuffle(uint32_t* arr, size_t num_tuples)
{
 ...
 */
void
random_unique_gen(uint32_t*& arr, size_t num_tuples)
{
...
void
dummy_initialize(uint32_t*& arr, size_t num_tuples) {
    for (size_t i = 0; i < num_tuples; i++) {
        arr[i] = i;
    }
}
...
int
create_relation_pk(uint32_t*& keys, uint32_t*& vals, size_t num_tuples)
{
  check_seed();

  keys = (uint32_t*)_mm_malloc(num_tuples * sizeof(int), 256);
  vals = (uint32_t*)_mm_malloc(num_tuples * sizeof(int), 256);
...
int create_relation_fk(uint32_t*& keys, uint32_t*& vals, size_t num_tuples, const uint32_t maxid)
{
  uint32_t i, iters, remainder;
  ...
  if (remainder > 0) {
    uint32_t* tuples = keys + maxid * iters;
...
int create_relation_fk(uint32_t*& keys, uint32_t*& vals, size_t num_tuples, const uint32_t maxid,
    uint32_t* d_keys, uint32_t* d_vals)
{
  uint32_t i, iters, remainder;
  ...
  for (i = 0; i < iters; i++) {
    uint32_t* tuples = keys + maxid * i;
    //random_unique_gen(tuples, maxid);
    memcpy(tuples, d_keys, sizeof(int) * maxid);
  }
  ...
  if (remainder > 0) {
    uint32_t* tuples = keys + maxid * iters;
    //random_unique_gen(tuples, remainder);
    memcpy(tuples, d_keys, sizeof(int) * remainder);
  }
